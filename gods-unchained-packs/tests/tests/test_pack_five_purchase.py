...
def test_purchase_valid(
    ...
    ## Valid cases
    # purchase rare pack
    tx = pack.functions.purchase(pack_types['Rare'], 1, accounts[2]).transact({'from': accounts[1], 'value': pack_prices['Rare']})
    purchaseID = 0
    ...
    assert purchased[2] == 0, "Check packType"
    ...
    assert purchased[6] == accounts[1], "purchaser is accounts[1]"
    ...
    assert logs[0]['args']['user'] == accounts[1], "PackPurchase user"
...
def test_purchase_max_count(
    ...
    pack.functions.purchase(pack_types['Shiny'], 2**15, accounts[2]).transact({'from': accounts[1], 'value': pack_prices['Shiny'] * 2**15})


def test_purchase_discounted(
        accounts,
        assert_tx_failed,
        ganache_mine_block,
        get_logs_for_event,
    ...
    # Purchase a pack with a discount, this shouldn't fail
...
def test_purchase_referrer_zero(
    ...
    tx = pack.functions.purchase(pack_types['Rare'], 1, zero_address).transact({'from': accounts[1], 'value': pack_prices['Rare']})
    logs = get_logs_for_event(processor.events.PaymentProcessed, tx)
    assert logs[0]['args']['cost'] == pack_prices['Rare'], "PaymentProcessed cost"
...
def test_purchase_different_count_vs_bundle_size(
        accounts,
        assert_tx_failed,
        ganache_mine_block,
        get_logs_for_event,
    ...
    tx = pack.functions.purchase(pack_types['Epic'], 1, accounts[2]).transact({'from': accounts[1], 'value': pack_prices['Epic'] * 1})
...
def test_purchase_not_enough_eth(
    ...
    assert_tx_failed(pack.functions.purchase(pack_types['Rare'], 1, accounts[2]), {'from': accounts[1], 'value': 1})
    assert_tx_failed(pack.functions.purchase(pack_types['Rare'], 1, accounts[2]), {'from': accounts[1], 'value': 0})
    assert_tx_failed(pack.functions.purchase(pack_types['Rare'], 1, accounts[2]), {'from': accounts[1], 'value': pack_prices['Rare'] - 1})


def test_purchase_buying_0_cards(
        accounts,
        assert_tx_failed,
        ganache_mine_block,
        get_logs_for_event,
    ...
    assert_tx_failed(pack.functions.purchase(pack_types['Rare'], 0, accounts[2]), {'from': accounts[1], 'value': pack_prices['Rare']})
...
def test_purchase_bad_pack_type(
    ...
    assert_tx_failed(pack.functions.purchase(5, 1, accounts[2]), {'from': accounts[1], 'value': pack_prices['Rare']})
    assert_tx_failed(pack.functions.purchase(0, 1, accounts[2]), {'from': accounts[1], 'value': pack_prices['Rare']})
    assert_tx_failed(pack.functions.purchase(2**8 - 1, 1, accounts[2]), {'from': accounts[1], 'value': pack_prices['Rare']})
...
def test_purchase_refer_self(
    ...
    assert_tx_failed(pack.functions.purchase(pack_types['Rare'], 1, accounts[1]), {'from': accounts[1], 'value': pack_prices['Rare']})
    assert_tx_failed(pack.functions.purchase(pack_types['Rare'], 1, accounts[0]), {'from': accounts[0], 'value': pack_prices['Rare']})
