...
const deployTestRig = async function (accounts) {
  ...
  // Deploy issuer and issuerEternalStorage
  const issuer = await Issuer.new(owner, resolver.address, { from: owner});
  const issuerEternalStorage = await IssuanceEternalStorage.new(owner, issuer.address);
  ...
  */
  ...
  // Deploy delegate approval eternal storage and contract

  const delegateApprovals = await DelegateApprovals.new(owner, zeroAddress);
  const delegateEternalStorage = await EternalStorage.new(owner, delegateApprovals.address);

  // Link the two delegate approval contract with its storage

  await delegateApprovals.setEternalStorage(delegateEternalStorage.address, { from: owner });

// Deploy the SystemStatus contract:

  const systemStatus = await SystemStatus.new(owner);
  ...
  // Update RewardsDistribution and RewardEscrow and SupplySchedule with the Synthetix address
  ...
  await supplySchedule.setSynthetixProxy(proxyForSynthetix.address, { from: owner })
  /*
